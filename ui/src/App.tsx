...
export default function App() {
  ...
  // Toggle state
  const [ingesting, setIngesting] = useState<boolean>(false);  // controls Start/Stop UI
  ...
  function enableAuto() {
    ...
    refreshTimer.current = window.setInterval(loadCoverage, 10000);
  }
  ...
  async function startIngestionRequest() {
    try {
      setStartMsg("");
      const payload = {
  ...
  async function toggleIngestion() {
    if (!ingesting) {
      // START
      await startIngestionRequest();
      setIngesting(true);
      setAutoRefresh(true);
      enableAuto();
      // kick an immediate coverage load
      loadCoverage();
    } else {
      // STOP (UI-side only; stops auto-refresh)
      setIngesting(false);
      setAutoRefresh(false);
      disableAuto();
      setStartMsg("Ingestion paused on UI; auto-refresh disabled.");
    }
  }

  // Initial load + attach/detach timer based on autoRefresh
  useEffect(() => {
    loadCoverage();
  }, []);
  useEffect(() => {
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 10, alignItems: "end" }}>
          <div>
            ...
            <input
              ...
              onChange={(e) => setTopSymbols(normalizeInt(e.target.value, 1, 200))}
              onBlur={(e) => setTopSymbols(normalizeInt(e.target.value, 1, 200))}
          ...
          <div>
            <button
              ...
              style={ingesting ? stopStyle : startStyle}
            >
