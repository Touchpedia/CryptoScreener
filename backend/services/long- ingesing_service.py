...
async def run_ingestion_job(symbol: str, timeframe: str) -> None:
    service = IngestionService()
    try:
        await service.init_exchange()
        await service.ingest(symbol, timeframe)
    except Exception as e:
        logger.error(f"Job failed for {symbol} {timeframe}: {str(e)}")
        raise
